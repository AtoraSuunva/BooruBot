...
module.exports.events.message = async (bot, message) => {
  ...
  if (booru.resolveSite(args[0]) || ['r', 'rand', 'random'].includes(args[0])) {
    ...
    if (
      !booru.resolveSite(args[1]) &&
      !['r', 'rand', 'random'].includes(args[1])
    ) {
  ...
  if (blacklistedTags)
    return message.channel.send(
      `Search contains blacklisted tag${
        blacklistedTags.length === 0 ? '' : 's'
  ...
  if (!['r', 'rand', 'random'].includes(args[1]) && !booru.resolveSite(args[1]))
    return message.channel.send(
      'That is not a supported site. Use `b!sites` to see them all.',
    )
        ...
        if (imgs[0] !== undefined) {
          for (let img of imgs) {
            if (
              ...
              ([null, undefined].includes(settings.minScore) ||
                Number.isNaN(img.score) ||
                img.score > settings.minScore)
            )
              validImgs.push(img)
          }
...
function randSearch(tags, settings, message) {
  return new Promise(async (resolve, reject) => {
    ...
    for (let site of randSites) {
      ...
      if (
        result === undefined ||
        result.imgs === undefined ||
        result.imgs[0] === undefined
      )
        continue
...
function compareArrays(arr1, arr2) {
  ...
  arr1.forEach(ele1 => {
    arr2.forEach(ele2 => {
      if (ele1 === ele2) matches.push(ele1)
    })
  })
...
async function postEmbed({
  ...
  let embed = new Discord.MessageEmbed({
    ...
    url: img.file_url,
    footer: {
      text: `${siteUrl} · ${imageNumber}/${numImages} · ${(
        (searchTime[0] * 1e9 + searchTime[1]) /
        1e6
      ).toFixed(2)}ms`,
  ...
  try {
    ...
    imgError = true /* who needs to catch shit */
  }
  ...
  embed.setDescription(
    `**Score:** ${img.score} | ` +
      ...
      `${path.extname(img.file_url).toLowerCase()}, ${
        headers ? fileSizeSI(headers.get('content-length')) : '? kB'
      }\n` +
      `**Tags:** ${tags}` +
...
module.exports.events.messageReactionAdd = async (bot, react, user) => {
  ...
  const settingsId = react.message.guild
